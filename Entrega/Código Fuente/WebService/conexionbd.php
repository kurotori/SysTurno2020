...
/**
* Comprueba la existencia de un usuario en el sistema. Devuelve 'false' si el usuario
* ya esta registrado
*/
function buscarUsuarioExiste($usuario_ci){
    $confirmacion = false;
    $conexion = GenerarConexion();
        try{
            // set the PDO error mode to exception
            $conexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $consulta = "SELECT count(*) FROM Usuario WHERE ci = :usuario_ci";
            $sentencia = $conexion->prepare($consulta);
